...
namespace geometry
{
	...
	// point, only valid for 2 and 3 dimensions!
	// TODO : generate compile error!
template <typename scalar, size_t dim>
struct Point {};

template <typename scalar>
struct Point<scalar,3> {
	scalar x;
	scalar y;
	scalar z;
};
...
template <typename scalar>
struct Point<scalar,2> {
	scalar x;
	scalar y;
};

template<typename scalar>
struct Point<scalar,1> {};

template<typename scalar>
struct Point<scalar,0> {};
...
template <typename scalar = double, size_t dim = 2>
struct Polygon : std::vector<Line<scalar,dim>> {};
